#include <stdio.h>
#include <math.h>
#include <stdlib.h>
/* chall6
int K1[272] = {

};
int K2[20] = {
0x00, 0x4D, 0x51, 0x50, 0xEF, 0xFB, 0xC3, 0xCF, 0x92, 0x45, 0x4D, 0xCF, 0xF5, 0x04, 0x40, 0x50,
0x43, 0x63, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
};
int search(const int a[],int n,int key){
    for(int i = 0;i<n;i++){
        if(a[i] == key){
            return i;
        }
    }
    return -1;
}
int main(){
    char flag[19];
    int v2;
    int v3;
    for(int i = 0;i < 19;i++){
        //v3 = abs(k[i] - i);
        //flag[i] = abs((K2[i] + i) - K1[i]);
        flag[i] = search(K1,272,(K2[i]));
        printf("%d %d\n",search(K1,272,(K2[i])));
    }

	printf("%s",flag);
	return 0;
}
*/




/*
unsigned char _rotr(unsigned char value,  unsigned char shift) {
    if ((shift &= 7) == 0)
        return value;
    return (value >> shift) | (value << (8 - shift));
}

unsigned char key[] = {
0x52, 0xDF, 0xB3, 0x60, 0xF1, 0x8B, 0x1C, 0xB5, 0x57, 0xD1, 0x9F, 0x38, 0x4B, 0x29, 0xD9, 0x26,
0x7F, 0xC9, 0xA3, 0xE9, 0x53, 0x18, 0x4F, 0xB8, 0x6A, 0xCB, 0x87, 0x58, 0x5B, 0x39, 0x1E, 0x00,
0x3B, 0xBD, 0x51, 0x7C, 0x69, 0xE8, 0xFF, 0xFF, 0xC4, 0x42, 0xAE, 0x83, 0x96, 0x17, 0x00, 0x00
};
int main(){
    unsigned char flag[0x20];
    unsigned char v1;
    int v2;
    int v3;
    for(int i = 0;i<0x20;i++){
        v1 = key[i]^i;
        flag[i] = _rotr(v1,i);
        printf("%x %x\n",v1,flag[i]);
        //flag[i] = v1>>i;
    }
    printf("%s",flag);

}
*/

int key[] = {
0xAC, 0xF3, 0x0C, 0x25, 0xA3, 0x10, 0xB7, 0x25, 0x16, 0xC6, 0xB7, 0xBC, 0x07, 0x25, 0x02, 0xD5,
0xC6, 0x11, 0x07, 0xC5
};
/*

imul eax,eax,FB
and eax,ff
*/
int main(){
    char flag[0x20];
    int v1;
    int v2;
    int v3;
    //0x41~0x7A
    int c = 0;
    while(c < 0x20){
        int i = 0;
        while(1){
            
            v1 = (i+33)*0xFB;
            v2 = v1 & 0xFF;
            if(v2 == key[c]){
                flag[c] = i+33;
                //printf("%x",v2);
                break;
            }
            if(i+33 == 126){
                flag[c] = '+';
                break;
            }
            i++;
        }
        c++;
    }
    printf("%s",flag);
    //printf("%s",flag);

}



































